...
from tensorflow.keras.layers import Dense, LSTM, Dropout, BatchNormalization, Conv1D, Input, Add, Bidirectional
...
ALL_TIME_MARKET_DATA_FILE = os.path.join(SCRIPT_DIR, "all_time_market_data_log.csv")
...
class ModelEnsemble:
    ...
    def build_single_model(self):
        ...
        # First bidirectional LSTM branch with residual connection
        lstm1 = Bidirectional(LSTM(128, return_sequences=True,
                                  kernel_regularizer=l2(0.001)))(x)
        bn1 = BatchNormalization()(lstm1)
        drop1 = Dropout(0.3)(bn1)
        
        # Second bidirectional LSTM branch
    ...
    if not os.path.exists(file_path):
        with open(file_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "Momentum", "VolatilityRange", "Hour", "Weekday", "NewsSentiment",
                "EMAShort", "EMALong", "EMARatio", "PriceChange", "WMA"
            ])
    ...
    if not os.path.exists(file_path):
        with open(file_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "AvgVolume", "Momentum", "VolatilityRange", "Hour", "Weekday",
                "NewsSentiment", "EMAShort", "EMALong", "EMARatio", "PriceChange", "WMA"
            ])

def log_trade(file_path, row):                      
    with open(file_path, "a", newline="") as f:
        writer = csv.writer(f)
        expected_columns = 32  # Number of expected columns
...
def log_full_history(file_path, row):
    with open(file_path, "a", newline="") as f:
        writer = csv.writer(f)
        expected_columns = 25  # Number of expected columns for full_history_log.csv
...
def compute_EMA(prices, span=12):
...
def compute_WMA(prices, period=10):
    ...
    subset = np.array(prices[-period:])
    return float(np.dot(subset, weights) / weights.sum())

def compute_slope(prices):
    """Compute price trend slope"""
    if not prices:
        return 0.0
...
def build_feature_vector(prices, sentiment, timestamp):
    """Construct feature vector from price history and metadata"""
    slope = compute_slope(prices)
    ma = np.mean(prices)
    std = np.std(prices)
    rsi = compute_RSI(prices)
    macd, signal = compute_MACD(prices)
    boll_sma, _, _ = compute_bollinger_bands(prices)
    atr = 0.0
    volume = 0.0
    momentum = prices[-1] - prices[-5] if len(prices) >= 5 else 0.0
    vol_range = (max(prices[-5:]) - min(prices[-5:])) if len(prices) >= 5 else 0.0
    hour = timestamp.hour
    weekday = timestamp.weekday()
    news = 0.5
    ema_short = compute_EMA(prices, span=5)
    ema_long = compute_EMA(prices, span=20)
    ema_ratio = (ema_short - ema_long) / ema_long if ema_long != 0 else 0.0
    price_change = (prices[-1] - prices[-2]) / prices[-2] if len(prices) > 1 and prices[-2] != 0 else 0.0
    wma = compute_WMA(prices, period=10)
...
def load_and_validate_data():
    """Load features and outcomes from both log files"""
    try:
        ...
        with open(LOG_FILE, "r") as f:
            ...
            for idx, row in enumerate(reader, 1):
                try:
                    ...
                    sentiment = float(row[16]) if len(row) > 16 else 0.5
                    timestamp = datetime.datetime.fromisoformat(row[0])
                    feat = build_feature_vector(prices, sentiment, timestamp)
                    features.append(feat)
                    outcome = 1 if row[8].strip().lower() == "win" else 0
                    outcomes.append(outcome)
                    if idx % 100 == 0:
                        print(f"Processed {idx} trade log rows...")
                except Exception as e:
                    if idx < 5:
                        print(f"Error processing trade log row {idx}: {e}")
                    continue

        print(f"Finished loading trade log features: {len(features)}")

        # --- Load full_history_log.csv ---
        print("\nLoading additional data from full history log...")
        rows = []
        try:
            with open(FULL_HISTORY_LOG_FILE, "r") as f:
                reader = csv.reader(f)
                header = next(reader, None)
                for row in reader:
                    try:
                        timestamp = datetime.datetime.fromisoformat(row[0])
                        prices = [float(p) for p in row[2].split(',')]
                        sentiment = float(row[3]) if len(row) > 3 else 0.5
                        rows.append((timestamp, prices, sentiment))
                    except Exception:
                        continue
        except FileNotFoundError:
            print("Full history log not found, skipping...")

        for i in range(len(rows) - 1):
            timestamp, prices, sentiment = rows[i]
            next_prices = rows[i + 1][1]
            feat = build_feature_vector(prices, sentiment, timestamp)
            outcome = 1 if next_prices[-1] > prices[-1] else 0
            features.append(feat)
            outcomes.append(outcome)
            if (i + 1) % 1000 == 0:
                print(f"Processed {i + 1} full history rows...")

        # --- Load all_time_market_data_log.csv ---
        print("\nLoading data from all time market history log...")
        try:
            with open(ALL_TIME_MARKET_DATA_FILE, "r") as f:
                reader = csv.reader(f)
                header = next(reader, None)
                market_rows = list(reader)
        except FileNotFoundError:
            print("All time market data log not found, skipping...")
            market_rows = []
        ...
        for i in range(len(market_rows) - 1):
            ...
            try:
                ...
                if len(closing_prices) >= HISTORY_WINDOW:
                    window = closing_prices[-HISTORY_WINDOW:]
                    sentiment = float(row[8]) if len(row) > 8 else 0.5
                    feat = build_feature_vector(window, sentiment, timestamp)
                    next_price = float(market_rows[i + 1][2])
                    outcome = 1 if next_price > close_price else 0
                    features.append(feat)
                    outcomes.append(outcome)

                if (i + 1) % 1000 == 0:
                    print(f"Processed {i + 1} market data rows...")
            except Exception:
                continue
        ...
        print(f"\nCombined dataset size: {len(features)}")
    ...
    try:
        ...
        while True:
            try:
                ...
                vol_range = (max(prices[-5:]) - min(prices[-5:])) if len(prices) >= 5 else 0
                ...
                # Prepare features
                feat_list = build_feature_vector(prices, sentiment, current_time)
                ...
                log_full_history(FULL_HISTORY_LOG_FILE, [
                    ...
                    momentum, vol_range, hour, weekday, news,
                    ema_short, ema_long, ema_ratio, price_change, wma
                ])
