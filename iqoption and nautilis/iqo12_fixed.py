...
class ModelEnsemble:
    ...
    def build_single_model(self):
        ...
        def custom_schedule(step):
            ...
            decay_progress = min(1.0, step_after_warmup / decay_steps)
            cosine_decay = 0.5 * (1 + tf.cos(np.pi * decay_progress))
            return min_lr + (initial_learning_rate - min_lr) * cosine_decay
        
        lr_schedule = tf.keras.optimizers.schedules.LearningRateSchedule()
        lr_schedule.__call__ = custom_schedule
    ...
    def train_ensemble(self, X, y):
        ...
        if self.feature_scaler is None:
            ...
            X = self.feature_scaler.fit_transform(X.reshape(-1, X.shape[-1])).reshape(X.shape)
        else:
    ...
    try:
        ...
        with open(FULL_HISTORY_LOG_FILE, 'r') as f:
            ...
            for row in reader:
                total_rows += 1
                try:
                    # Expected format: Timestamp, Asset, PriceHistoryString, ...
                    if len(row) >= 13:  # At least 13 columns for features
                        feature_values = [float(x) for x in row[4:17]]  # Columns 5-17 contain feature values
                        features.append(feature_values)
                        valid_rows += 1
                except (ValueError, IndexError) as e:
                    if total_rows < 5:  # Only show first few errors
                        print(f"Error processing row {total_rows}: {str(e)}")
                    continue
                
                if total_rows % 1000 == 0:
                    print(f"Processed {total_rows} rows, found {valid_rows} valid feature sets...")
        ...
        if len(features) < MIN_TRADES_FOR_TRAINING:
            print(f"WARNING: Not enough valid features for training. Need at least {MIN_TRADES_FOR_TRAINING}, found {len(features)}")
            return None, None
        ...
        with open(LOG_FILE, 'r') as f:
            ...
            for row in reader:
                total_rows += 1
                try:
                    if len(row) >= 9:  # At least 9 columns
                ...
                if total_rows % 1000 == 0:
                    print(f"Processed {total_rows} rows, found {valid_rows} valid outcomes...")
        
        print(f"\nOutcome loading complete:")
        print(f"Total rows processed: {total_rows}")
        print(f"Valid outcomes extracted: {len(outcomes)}")
        
        # Validate data alignment
        if len(features) != len(outcomes):
            print(f"WARNING: Feature count ({len(features)}) doesn't match outcome count ({len(outcomes)})")
            min_len = min(len(features), len(outcomes))
            features = features[:min_len]
            outcomes = outcomes[:min_len]
            print(f"Truncated to {min_len} matched pairs")
