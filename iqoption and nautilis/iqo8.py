...
def compute_RSI(prices, period=14): # Takes list of prices
    ...
    # Calculate average gains and losses using Wilder's smoothing method (SMA for first value)
    avg_gain = np.mean(gains[:period]) if len(gains) >= period else (np.sum(gains) / period if len(gains) > 0 else 0)
    # For losses, we need their absolute values for averaging
...
def compute_EMA(prices, period): # Takes list of prices
    ...
    try:
        ...
        if len(prices) < period: 
            # If not enough data for a full EMA, returning a simple moving average for the available data
            # or an average of the prices if fewer than period. This EMA will be shorter.
...
def compute_MACD(prices, short_period=12, long_period=26, signal_period=9): # Takes list of prices
    if not prices or len(prices) < long_period: # Need enough data for the longest EMA
        return 0.0, 0.0
    try:
        ...
        if not ema12 or not ema26 or len(ema12) != len(ema26): 
            # This can happen if compute_EMA_simple_for_MACD is not robust for all price list lengths
            return 0.0, 0.0
...
def compute_EMA_simple_for_MACD(prices, period): # Ensures full length for MACD if pandas not used
    ...
    ema = [0.0] * len(prices) # Initialize with zeros to match length of prices
    if not ema: return []
    
    if len(prices) < period : # Handle cases with insufficient data for a full period SMA start
        if prices:
            # If fewer than 'period' prices, use all available for a simple average as starting point
            # and then proceed with EMA logic for remaining, or just return SMA for all
            initial_avg = np.mean(prices)
            for i in range(len(prices)):
                if i == 0:
                    ema[i] = initial_avg
                else:
                    k = 2 / (min(i + 1, period) + 1) # Adjust k for shorter initial periods
    ...
    ema[0] = np.mean(prices[:period]) # Start with SMA of the first 'period' prices for the first valid EMA point
                                     # Actually, to match pandas adjust=False, first value IS first price for many definitions.
                                     # Let's stick to a simple iterative EMA for full length:
...
def retrain_supervised_model(log_file, state_dim=STATE_DIM, action_dim=ACTION_DIM, epochs=5):
    ...
    try:
        ...
        # Read only necessary columns to be robust.
        # Explicitly handle potential errors during CSV parsing for this specific file.
        try:
            data = pd.read_csv(log_file, usecols=lambda c: c in cols_to_read)
        except pd.errors.ParserError as pe:
            print(f"Pandas ParserError reading {log_file}: {pe}. Attempting to skip bad lines.")
            try:
                # For CParserError, skipping bad lines might help if some rows are malformed
                data = pd.read_csv(log_file, usecols=lambda c: c in cols_to_read, on_bad_lines='skip')
            except Exception as e_skip:
                print(f"Failed to read {log_file} even with on_bad_lines='skip': {e_skip}")
                return None
        except ValueError as ve: # Might happen if usecols results in no columns if file is totally empty/corrupt after header
             print(f"ValueError reading {log_file} (check if file is empty or headers mismatch severely): {ve}")
             return None
        ...
        for col in feature_cols_for_training:
            if col not in data_filtered.columns:
                print(f"Warning: Feature column '{col}' not found in filtered data. Filling with 0.0.")
                data_filtered[col] = 0.0 
                missing_features.append(col)
        if missing_features:
             print(f"Summary: The following feature columns were missing and filled with 0: {missing_features}")
        
        X = data_filtered[feature_cols_for_training].values.astype(np.float32)
        y = data_filtered["Profit"].values.astype(np.float32)

        if X.shape[0] < 10: # Need some data for training
...
def main():
    ...
    try:
        ...
        # --- Main Persistent Loop for Continuous Logging and Periodic Trading ---
        while main_loop_active and trade_attempt_count < TRADING_ITERATIONS:
            current_iso_timestamp = datetime.datetime.now().isoformat()
            continuous_candles_data = None
            continuous_closing_prices_list = []
            
            # Declare indicator variables for this scope
            cont_slope, cont_ma, cont_std, cont_rsi, cont_macd, cont_macd_signal = (0.0,) * 6
            cont_trader_sentiment, cont_boll_sma, cont_atr, cont_avg_volume = (0.0,) * 4
            cont_hour, cont_weekday = (0, 0)
            cont_news_sentiment = 0.5
            price_history_str_cont = ""


            # 1. Continuous Data Fetching and Logging (Every Second)
            try:
                I_want_money.start_candles_stream(ASSET, TIMEFRAME, NUM_CANDLES)
                time.sleep(0.1) # Minimal sleep, let get_realtime_candles manage timing
                continuous_candles_data = I_want_money.get_realtime_candles(ASSET, TIMEFRAME)
                # Consider stopping stream if it's not auto-managed:
                # I_want_money.stop_candles_stream(ASSET, TIMEFRAME) 
                ...
                if continuous_candles_data and isinstance(continuous_candles_data, dict) and any(continuous_candles_data):
                    ...
                    price_history_str_cont = "NO_CANDLE_DATA"
                    cont_trader_sentiment = 0.5 # Default fallback
                    # Other indicators remain 0.0 or default
                ...
                if I_want_money and not I_want_money.check_connect():
                    ...
                    except Exception as e_reconnect_cont:
                        print(f"Exception during reconnect in continuous log: {e_reconnect_cont}. Stopping bot.")
                        main_loop_active = False
                time.sleep(1) # Wait a bit before next attempt if error

            if not main_loop_active: break # Exit if reconnect failed

            # 2. Trade Execution Logic (Triggered periodically)
            seconds_since_last_trade_attempt += 1
            if seconds_since_last_trade_attempt >= INTER_TRADE_WAIT_SECONDS:
                seconds_since_last_trade_attempt = 0 # Reset counter
                ...
                if not (continuous_candles_data and continuous_closing_prices_list):
                    print("Skipping trade attempt due to lack of fresh data from continuous logging.")
                else:
                    # Use the indicators calculated in the continuous logging section
                    ...
                    if trade_status:
                        ...
                        max_wait_trade = DURATION * 60 + 20 # Increased buffer slightly
                        api_trade_result = None; stop_loss_triggered_flag = False; take_profit_triggered_flag = False

                        while time.time() - trade_wait_start < max_wait_trade:
                            # No separate full_history logging here; main 1-sec loop covers it.
                            # Only focus on trade result and risk management.
                            current_candles_trade_wait = I_want_money.get_realtime_candles(ASSET, TIMEFRAME) # Fetch fresh data
                            ...
                            if current_prices_trade_wait:
                                ...
                                if not take_profit_triggered_flag and is_take_profit_triggered(live_price, entry_price_for_trade, action_type=action_str):
                                    ...
                                    take_profit_triggered_flag = True # Actual profit determined by check_win
            ...
            if not main_loop_active: break # Exit if flag set due to critical error
            time.sleep(1) # Ensure the main loop attempts to run every second
