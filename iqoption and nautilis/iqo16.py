...
class ModelEnsemble:
    ...
    def build_single_model(self):
        ...
        lstm2 = Bidirectional(LSTM(32, return_sequences=False,
...
def log_trade(file_path, row):                      
    with open(file_path, "a", newline="") as f:
        writer = csv.writer(f)
        expected_columns = 27  # Number of expected columns
...
def log_full_history(file_path, row):
    with open(file_path, "a", newline="") as f:
        writer = csv.writer(f)
        expected_columns = 20  # Number of expected columns for full_history_log.csv
